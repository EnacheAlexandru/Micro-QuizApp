            ...
            <div>
              <div className="m-5 flex flex-col items-center gap-2">
                {leaderboard.players.map((item) => {
                  position++;
                  return <React.Fragment key={item.username}>
                      <div className={`w-3/4 p-3 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700
                      ${leaderboard.page === 1 ? position === 1 ? 'bg-yellow-400' : position === 2 ? 'bg-gray-400' : position === 3 ? 'bg-yellow-600' : '' : ''}
                      `}>
              ...
              <div className="font-bold text-2xl flex justify-center">Be the first to answer a question and get to the leaderboard!</div>
            )
          }
        </div>
      );
